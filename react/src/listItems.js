...
class ListItems extends React.Component {
    constructor(props) {
        ...
        this.enlargeItems = this.enlargeItems.bind(this);
    ...
    enlargeItems(e) {
        commander.send({ commandName: 'highlightFeature', featureId: e.target.parentElement.id, color: '#00FFFF' });
    }

    shrinkItems(e) {
        commander.send({ commandName: 'highlightFeature', featureId: e.target.parentElement.id, color: undefined });
    }

    deleteItems(e) {
        if (confirm('Are you sure to delete this item?')) {
            let itemInfo = '';
            commander.send({ commandName: 'deleteShapes', featureIds: [e.target.parentElement.id] });
            for (let item of global.itemsInfo) {
                if (item.featureId === e.target.parentElement.id) {
                    itemInfo = item;
                }
            }
            let itemIndex = global.itemsInfo.indexOf(itemInfo);
            global.itemsInfo.splice(itemIndex, 1);
        }
    }

    zoomToItems(e) {
        commander.send({ commandName: 'centerOnShape', featureId: e.target.parentElement.id });
    }
