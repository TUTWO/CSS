!function () {
    ...
    document.getElementById("extents").addEventListener("blur", function () {
        ...
        var text = document.getElementById("extents").value;
        if (!text) {
            document.getElementById("inputExtent").innerHTML = "Please Input!";
            document.getElementById("extents").style.border = "1px solid red";
        }
    })
    ...
    var handles =
    {
        "drawSymbol": function addItem(data) {
            ...
            radio.addEventListener("click", showhideSymbol)
    ...
    function enlargeSymbol(evt, isHighlight) {
        mouseEvent = evt.type;
        isHighlight = (mouseEvent == "mouseover" ? true : false);
    ...
    function showhideSymbol(evt) {
        var commandName = evt.target.checked ? "showFeature" : "hideFeature";
        sendMessages({ commandName: commandName, featureIds: [evt.target.parentElement.id] });
